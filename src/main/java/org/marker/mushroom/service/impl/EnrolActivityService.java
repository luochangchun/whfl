...
public class EnrolActivityService extends BaseService {
	...
	public ResultMessage createEnrolActivity(EnrolActivity enrolActivity) {
		...
		m = p.matcher(str);
		b = m.matches();
		if (!b) return new ResultMessage(false, "号码格式不正确！请重新输入！");
		...
		enrolActivity.setTime(new Date());
		if (!commonDao.save(enrolActivity)) {
	...
	public Page findByPage(int currentPageNo, int pageSize) {
		final String sql =
			"select id,name,phone,appellation,time from " + config.getPrefix() + "enrolactivity order by time desc";
		return commonDao.findByPage(currentPageNo, pageSize, sql);
	}

	//删除操作
	public boolean EnrolActivityDelete(String rid) {
		return commonDao.deleteByIds(EnrolActivity.class, rid);
	}
