 ...
 * @version 1.0
 */
@Service(Services.VOLUNTEERPERSONAL)
public class VolunteerPersonalService extends BaseService
{
	...
	{
		final String sql = " select m.*,d.value from " + config.getPrefix() + "volunteer_personal m left join " + config.getPrefix()
	...
	{
		final String sql = " select m.name,m.idCard,m.phone,m.description,d.value,m.applyTime,m.like,m.culture from "
				+ config.getPrefix() + "volunteer_personal m left join " + config.getPrefix()
				+ "dictionaries d on m.region=d.id and d.name='region' and d.type='volunteer' order by m.time desc ";
