...
function addContent(obj) {
	...
	// 视图切换回调，资源回收操作
	if (window.view_callback)
		window.view_callback();
...
//字数计算及提醒
function textNumCal(e) {
	//	var $length = $(e).val().replace(/[^\x00-\xff]/g, "**").length;
	var $length = $(e).val().length;
	var $data = $(e).data();
	if ($length > 0) {
		var $left = parseInt($data.max) - $length;
		if ($length <= $data.max) {
			$($data.target).html("剩余" + $left + "个字符。").css("color", "green");
		} else {
			$($data.target).html("已经超出最大限制！").css("color", "red");
		}
	} else {
		$($data.target).empty();
	}
...
//上传图片模态框控制
function frameDialogShow() {
	$("#showFrameDialog").show();
}
function frameDialogHide() {
	$("#showFrameDialog").hide();
}
//加载图片
function imgChange() {
	var _val = $('#icon').attr('value');
	if (_val) {
		$(".input_for_img .img-box").html(
				"<img src='/" + _val
						+ "' style='display:block;width:100%;height:auto;'>");
	} else {
		$(".input_for_img .img-box")
				.html(
						"<img src='images/timg.png' style='display:block;width:100%;height:auto;'>");
	}
}
//上传图片
function upload(obj) {
	...
	zoom.showConfirmDialog("确定提交数据吗？", "消息提示", function() {
		var options = {
			...
			success : function(data) {
				...
				if (dataJson.status) {
					if ($('#icon')) {
						$('#icon').attr('value', dataJson.param);
						imgChange();
					}
					zoom.showMessageDialog(dataJson.message, "消息提示", 1000);
					refreshContentByURL($(obj).attr("return"), $(obj).attr(
							"name"));
				} else {
					if ('101' == dataJson.code) {// 表示访问正常页面，但会话失效
						zoom.showMessageDialog(dataJson.message + "<br/>",
								"消息提示", 1500, function() {
...
function flreject(obj) {
	zoom.showTextareaDialog("审核不通过！", "驳回理由:", function() {
		var reject = $("#showTextareaDialogInput").val();
		if (reject === null || reject === "") {
			zoom.closeTextareaDialog();
			zoom.showMessageDialog("驳回理由不能为空！", "消息提示", 1500);
		} else {
			$("#reject").val(reject);
			zoom.closeTextareaDialog();
	...
	zoom.showConfirmDialog("确定提交数据吗？", "消息提示", function() {
		var options = {
			...
			success : function(data) {
				zoom.closeConfirmDialog();//关闭确定提示
				if (data == null) {
					zoom.showMessageDialog("数据格式不正确或超长！", "消息提示", 1500);
					return;
				}
				if (data.status) {
					...
					if ('101' == data.code) {// 表示访问正常页面，但会话失效
						zoom.showMessageDialog(data.message + "<br/>", "消息提示",
								1500, function() {
									window.location.href = "login.do";// 回调中刷新界面
								});
					} else {
		...
		zoom.showConfirmDialog("确定删除吗？", "消息提示", function() {
			$.getJSON(action, list, function(data) {
				if (data.status) {
					...
					zoom.showMessageDialog("批量删除失败!<br/>" + data.message,
							"消息提示", 1500);
					zoom.closeConfirmDialog();
				}
			});
...
zoom.closeMessageDialog = function() {
	...
	if (currentDialog != null)
		window.document.body.removeChild(currentDialog);
...
zoom.showMessageDialog = function(content, title, time, callback) {
	...
	var htmlText = "<div class=\"dialogFullScreen\"></div>"
			...
			+ "<div class=\"dialogPanel\">" + "<h1>" + title + "</h1>"
			+ " <div class=\"showMessageDialogPanel\">"
			+ "<div class=\"dialogPanelContent\">" + content + " </div>"
			+ " </div>" + " <div class=\"dialogButton\">"
			+ "<button id=\"dialogButtonCancel\">取 消</button>" + "  </div>"
			+ "  </div>" + " </div>";
...
zoom.closeInputDialog = function() {
	...
	if (showInputDialog != null)
		window.document.body.removeChild(showInputDialog);
};
...
/* 输入框框 */
zoom.closeTextareaDialog = function() {
	var showTextareaDialog = document.getElementById("showTextareaDialog");
	if (showTextareaDialog != null)
		window.document.body.removeChild(showTextareaDialog);
};
zoom.showTextareaDialog = function(content, title, fun) {
	if (title == null)
		title = "消息提示";
	var dialog_div = document.createElement("div");
	$(dialog_div).attr("id", "showTextareaDialog");
	$(dialog_div).addClass("dialog");
	var htmlText = "<div id=\"showTextareaDialog\" class=\"dialog\">"
			+ "<div class=\"dialogFullScreen\"></div>"
			+ "<div class=\"dialogContent dialog_style_content\">"
			+ "<div class=\"dialogPanel\">"
			+ "<h1>"
			+ title
			+ "</h1>"
			+ " <div class=\"showTextareaDialogInput\">"
			+ "	<textarea id=\"showTextareaDialogInput\" style='width: 100%;height: 100px;overflow: auto;resize:none' oninput='textNumCal(this)' data-target='#num' data-max='255'>"
			+ content + "</textarea> <span id='num'></span>" + " </div>"
	...
	$(dialog_div).html(htmlText);
	window.document.body.appendChild(dialog_div);
	$("#showTextareaDialogInput").focus();// 设置焦点
	$("#showTextareaDialogInput").bind("keyup", function(event) {
		if (event.keyCode == 13)
			fun();
	});
	$("#dialogButtonConfirm").bind("click", fun);
...
/* 输入框框 */
zoom.closeCourseDialog = function() {
	var showCourseDialog = document.getElementById("showCourseDialog");
	if (showCourseDialog !== null)
		window.document.body.removeChild(showCourseDialog);
};
zoom.showCourseDialog = function(fun) {
	var title = "线下课程录入";
	var dialog_div = document.createElement("div");
	$(dialog_div).attr("id", "showCourseDialog");
	$(dialog_div).addClass("dialog");
	var htmlText = "<div id=\"showCourseDialog\" class=\"dialog\">"
			+ "<div class=\"dialogFullScreen\"></div>"
			+ "<div class=\"dialogContent dialog_style_content\">"
			+ "<div class=\"dialogPanel\">"
			+ "<h1>"
			...
			+ "</h1>"
			+ "<div class=\"showInputDialogPanel\">"
			+ "<div class=\"dialogPanelContent\">课程名称：<input type=\"text\" id=\"courseName\" class=\"input_for_login\" /></div>"
			+ " </div>"
			+ "<div class=\"showInputDialogPanel\">"
			+ "<div class=\"dialogPanelContent\">课程积分：<input type=\"text\" id=\"courseIntegral\" value=\"2\" class=\"input_for_login\" /></div>"
			+ " </div>" + " <div class=\"showInputDialogInput\">" + "	"
			+ " </div>" + "<div class=\"dialogButton\">"
			+ "<button id=\"courseButtonConfirm\">确 定</button> &nbsp; &nbsp; "
			+ "<button id=\"courseButtonCancel\">取 消</button>" + " </div>"
			+ "</div>" + "</div>" + " </div>";
	$(dialog_div).html(htmlText);
	window.document.body.appendChild(dialog_div);
	$("#courseName").focus();// 设置焦点
	$("#courseName").bind("keyup", function(event) {
		if (event.keyCode == 13)
			fun();
	});
	$("#courseButtonConfirm").bind("click", fun);
	$("#courseButtonCancel").bind("click", function() {
...
zoom.closeFrameDialog = function() {
	...
	if (showDialog != null)
		window.document.body.removeChild(showDialog);
};
zoom.showFrameDialog = function(title, width, height, content) {
	...
	var htmlText = "<div class=\"clear_zone\"></div>"
			+ "<div class=\"frame_window\" style=\"width:" + width
			+ "px;margin-left:-" + (width / 2) + "px; height: " + height
			+ "px; margin-top: -" + (height / 2) + "px;" + "\">"
			+ "<div class=\"frame_bar\"><span class=\"title\">" + title
			+ "</span><div class=\"frame_exit\"></div></div>"
			+ "<div class=\"frame_content\" style=\"height:" + (height - 36)
			+ "px;\"><div class=\"frame_margin markdown\">" + content
			+ "</div></div>" + "</div>";
	$(dialog_div).html(htmlText);
	window.document.body.appendChild(dialog_div);
